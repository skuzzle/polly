...
public class RemindManagerImpl extends AbstractDisposable implements RemindManager {
    ...
    private final Timer remindScheduler;
    private final Map<Integer, RemindTask> scheduledReminds;
    private final Map<String, RemindFormatter> specialFormats;
    private final Map<String, RemindEntity> sleeping;
    private final ActionCounter actionCounter;
    private final ActionCounter messageCounter;
    private final FormatManager formatter;
    private final RemindDBWrapper dbWrapper;
    ...
    public synchronized void addRemind(User executor, final RemindEntity remind,
        ...
        this.logger.info("Adding " + remind + ", schedule: " + schedule); //$NON-NLS-1$ //$NON-NLS-2$
        ...
        if (remind.isOnAction()) {
            this.logger.trace("Storing remind as on return action."); //$NON-NLS-1$
            this.actionCounter.put(remind.getForUser());
        }
        if (remind.isMessage()) {
            this.logger.trace("Storing remind as leave message."); //$NON-NLS-1$
    ...
    public void deleteRemind(int id) throws DatabaseException {
        ...
        this.logger.trace("Deleting remind with id " + id); //$NON-NLS-1$
    ...
    public synchronized void deleteRemind(RemindEntity remind) throws DatabaseException {
        if (remind != null) {
            ...
            this.logger.warn("tried to delete non-existent remind."); //$NON-NLS-1$
        }
    }
    ...
    public void deleteRemind(User executor, int id)
        ...
        this.logger.debug("User '" + executor + " wants to delete remind with id " + id); //$NON-NLS-1$ //$NON-NLS-2$
        RemindEntity remind = this.dbWrapper.getRemind(id);
        checkRemind(executor, remind, id);
        this.deleteRemind(remind);
    }
    ...
    public synchronized void deliverRemind(RemindEntity remind, boolean checkIdleStatus)
        ...
        User forUser = getUser(remind.getForUser());

        this.logger.info("Trying to deliver " + remind + " for " + forUser); //$NON-NLS-1$ //$NON-NLS-2$
        try {
            if (remind.isMail()) {
                deliverNowMail(remind, forUser, false);
            } else {
                this.logger.trace("Remind is to be delivered in IRC. Checking user state"); //$NON-NLS-1$
                boolean idle = isIdle(forUser) && checkIdleStatus;
                boolean online = this.irc.isOnline(forUser.getCurrentNickName());
                this.logger.trace("Idle state: " + idle + ", online state: " + online); //$NON-NLS-1$ //$NON-NLS-2$


                if (!online || idle) {
                    deliverLater(remind, forUser, idle, online);
                } else {
                    deliverNowIrc(remind, forUser, online);
                }
            }
        } finally {
            this.logger.trace("Now deleting " + remind); //$NON-NLS-1$
            this.deleteRemind(remind);
        }
    }
    ...
    public void deliverLater(final RemindEntity remind, User forUser, boolean wasIdle,
        ...
        this.logger.trace("Delivering later. Checking if remind schould be delivered as mail"); //$NON-NLS-1$
        ...
        this.logger.trace("As Mail: " + asMail); //$NON-NLS-1$
        this.logger.trace("Double-delivery: " + doubleDelivery); //$NON-NLS-1$

        if (asMail && wasIdle) {
            // user was idle and wanted email notification
            deliverNowIrc(remind, forUser, online);
            deliverNowMail(remind, forUser, wasIdle);
        } else if (asMail) {
            ...
            if (doubleDelivery) {
                ...
                addRemind(forUser, onAction, false);
            }
        } else {
            ...
            addRemind(forUser, message, false);
        }
    }
    ...
    public void deliverNowIrc(RemindEntity remind, User forUser, boolean online) {
        ...
        this.logger.trace("Delivering " + remind + " now in IRC"); //$NON-NLS-1$ //$NON-NLS-2$
        RemindFormatter formatter = getFormat(forUser);
        ...
        if (remind.isOnAction()) {
            this.logger.trace("Remind was onAction. Removing it from onActionSet"); //$NON-NLS-1$
        ...
        putToSleep(remind, forUser);
        checkTriggerAutoSnooze(forUser);
    }
    ...
    public void deliverNowMail(RemindEntity remind, User forUser, boolean wasIdle)
                throws DatabaseException, EMailException {
        this.logger.trace("Delivering " + remind + " now as mail"); //$NON-NLS-1$ //$NON-NLS-2$

        String mail = ((StringType) forUser.getAttribute(MyPlugin.EMAIL)).getValue();
        if (mail.equals(MyPlugin.DEFAULT_EMAIL)) {
            this.logger.warn("Destination user has no valid email address set"); //$NON-NLS-1$
    ...
    public void scheduleRemind(RemindEntity remind, Date dueDate) {
        this.logger.trace("Scheduling remind " + remind + ". Due date: " + dueDate); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public void cancelScheduledRemind(int id) {
        this.logger.trace("Cancelling scheduled remind with id " + id); //$NON-NLS-1$
    ...
    public void putToSleep(RemindEntity remind, User forUser) {
        this.logger.trace("Remembering " + remind + " for snooze"); //$NON-NLS-1$ //$NON-NLS-2$
        ...
        this.logger.trace("Snooze time for " + forUser + ": " + sleepTime); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public RemindEntity cancelSleep(String forUser) {
        this.logger.trace("Cancelling snooze for user " + forUser); //$NON-NLS-1$
        synchronized (this.sleeping) {
    ...
    public RemindEntity snooze(User executor, Date dueDate)
        ...
        this.logger.trace("User " + executor + " requested snooze"); //$NON-NLS-1$ //$NON-NLS-2$
        ...
        // running
        if (dueDate == null) {
            this.logger.trace("No duedate given. Calculating runtime of remind to snooze."); //$NON-NLS-1$
            long runtime = existing.getDueDate().getTime() -
                existing.getLeaveDate().getTime();
            dueDate = new Date(Time.currentTimeMillis() + runtime);
            this.logger.trace("Remind runtime is: " + this.formatter.formatTimeSpanMs(runtime)); //$NON-NLS-1$
        }

        RemindEntity newRemind = existing.copyForNewDueDate(dueDate);
        addRemind(executor, newRemind, true);
        return newRemind;
    }
    ...
    public RemindEntity toggleIsMail(User executor, int id)
        ...
        this.logger.trace("Toggeling delivery of " + remind); //$NON-NLS-1$
        checkRemind(executor, remind, id);
        ...
        this.logger.trace("New delivery type: " + (remind.isMail() ? "Mail" : "IRC")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        return remind;
    }
    ...
    public User getUser(String nickName) {
        this.logger.trace("Getting user for name '" + nickName + "'"); //$NON-NLS-1$ //$NON-NLS-2$
        User u = this.userManager.getUser(nickName);
        if (u == null) {
            this.logger.trace("User is unknown, creating new one"); //$NON-NLS-1$
    ...
    public RemindEntity modifyRemind(User executor, int id, final Date dueDate, final String msg)
            throws CommandException, DatabaseException {
        this.logger.trace("User " + executor + " requested to modify remind with id " + id); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public void checkRemind(User user, RemindEntity remind, int id)
        ...
        this.logger.trace("Checking for sufficient rights of user " + user + " for " + remind); //$NON-NLS-1$ //$NON-NLS-2$
        if (remind == null) {
    ...
    public void traceNickChange(IrcUser oldUser, final IrcUser newUser) {
        this.logger.trace("tracing nickchange " + oldUser + " -> " + newUser); //$NON-NLS-1$ //$NON-NLS-2$
        User oldForUser = getUser(oldUser.getNickName());
        final BooleanType track = (BooleanType) oldForUser.getAttribute(
            MyPlugin.REMIND_TRACK_NICKCHANGE);

        if (!track.getValue()) {
            this.logger.trace("User doesnt want this nickchange to be tracked"); //$NON-NLS-1$
            return;
        }
        ...
        User newForUser = getUser(newUser.getNickName());
        ...
        try {
            ...
            if (sleeping != null) {
                putToSleep(sleeping, newForUser);
            }
    ...
    public void rescheduleAll() {
        this.logger.trace("Scheduling all existing reminds for their duedate"); //$NON-NLS-1$
        List<RemindEntity> allReminds = this.dbWrapper.getAllReminds();
        synchronized (this.scheduledReminds) {
            for (RemindEntity r : allReminds) {
                this.logger.trace("Scheduling remind " + r + ". Due date: " + r.getDueDate()); //$NON-NLS-1$ //$NON-NLS-2$

                if (r.getDueDate().getTime() < 0) {
                    this.logger.warn("Skipping " + r + " because of negative due date"); //$NON-NLS-1$ //$NON-NLS-2$
                    continue;
                }
                ...
                try {
                    ...
                    this.logger.error("Error while scheduling " + r, e); //$NON-NLS-1$
                }
            }
        }
    }
