...
var FEATURE_LOGIN_INTEGRATION = true; // login code sharing. WARNING: if you turn this off, venad has to be set in RX settings manually
...
function settingIntegration() {
  ...
  content += '<td rowspan="3" style="vertical-align:middle; text-align:center"><input tabindex="300" class="Button" type="button" id="savePolly" value="{0}"/></td>'.format(MSG_STORE_SETTINGS);
  content += '</tr>';
  content += '<tr><td>{0}</td><td><input tabindex="256" class="text" type="password" id="pollyPw"/></td></tr>'.format(MSG_POLLY_PW);
  content += '<tr><td>{0}</td><td><input tabindex="257" class="text" type="text" id="self"/></td></tr>'.format(MSG_VENAD);
...
function saveOrionSettings() {
  ...
  GM_setValue(PROPERTY_ORION_SELF, self);
...
function showChanges(resultEntries, isTop50) {
  ...
  for (var i = 0; i < resultEntries.length; ++i) {
      ...
      if (entry.previousRank == -1 || rankDiff == 0) {
          rankText = " -- {0}".format(MSG_NO_CHANGE);
      } else if (rankDiff < 0) {
          rankText = '<span style="color:green"> +{0} (vorher: {1})</span>'
              .format(-rankDiff, entry.previousRank);
      } else if (rankDiff > 0) {
          rankText = '<span style="color:read"> -{0} (vorher: {1})</span>'
              .format(rankDiff, entry.previousRank);
      ...
      if (entry.previousPoints == -1 || pointsDiff == 0) {
          pointText = " -- {0}".format(MSG_NO_CHANGE);
      } else if (pointsDiff > 0) {
          rankText = '<span style="color:green"> +{0} (vorher: {1})</span> {3}'
              .format(pointsDiff, entry.previousPoints, entry.previousDate);
      } else if (pointsDiff < 0) {
          rankText = '<span style="color:red"> {0} (vorher: {1})</span> {3}'
              .format(pointsDiff, entry.previousPoints, entry.previousDate);
      }
...
function newsGui() {
  ...
  newContent += '<tr style="">\n';
  newContent += '<td class="nfo" colspan="5">{0}</td><td style="text-align:right;padding-top:0px" class="nfo"><input type="checkbox" id="toggleOrionNews"/>{1} | <a id="toggleSettings" href="#">{2}</a></td>\n'.format(MSG_SKY_NEWS, MSG_TURN_ON, MSG_PREFERENCES);
  newContent += '</tr>\n';
...
function handleInsertCode(property, oldVal, newVal) {
  ...
  if (newVal) {
      requestJson(API_REQUEST_CODE, {}, function (result) {
          $('input[name="ucode"]').val(result.code).focus().select();
      });
  } else {
  ...
  if (!enable) {
      ...
      requestNewsForQuadrant(getMaxNewsEntries(), sector.quadName, function (entries) {
          ...
          $.each(entries, function (idx, entry) {
              ...
              switch (entry.type) {
              case NEWS_FLEET_SPOTTED:
                  newContent += '<span class="hover" x="{0}" y="{1}" title="Reporter: {2} um {3}">Flotte: <b>{4}</b> bei {5} ({3})</span>' 
                      .format(sector.x, sector.y, entry.reporter, entry.date, entry.subject.ownerName, location(sector, false));
                      ' (' + entry.date + ')</span>';
                  newContent += "<br/>";
                  break;
              case NEWS_PORTAL_MOVED:
